...
std::vector<typename Observable::observable_type> MetropolisParallel<ConfigurationType, Step, RandomNumberGenerator>::do_parallel_metropolis_simulation(const Parameters& parameters, const TemperatureType& beta)
{
  ...
  do_parallel_metropolis_simulation<Observable>(parameters, beta, measurements_accumulator);
...
std::vector<std::vector<typename Observable::observable_type> > MetropolisParallel<ConfigurationType, Step, RandomNumberGenerator>::do_parallel_metropolis_simulation(const Parameters& parameters, InputIterator first_beta, InputIterator last_beta)
{
  std::vector<std::vector<typename Observable::observable_type> > results;
  for (InputIterator beta = first_beta; beta != last_beta; ++beta)
  {
    results.push_back(do_parallel_metropolis_simulation(parameters, *beta));
...
void MetropolisParallel<ConfigurationType,Step,RandomNumberGenerator>::do_parallel_metropolis_simulation(const Parameters& parameters, const TemperatureType& beta, Accumulator& measurement_accumulator)
{
  ...
  for (unsigned int run = 0; run < parameters.run_number; ++run)
  {
    ConfigurationType* copied_configuration = new ConfigurationType(*(this->get_config_space()));
    run_simulations.push_back(new Metropolis<ConfigurationType, Step, RandomNumberGenerator>(copied_configuration));
    run_simulations[run]->set_random_seed(this->get_random_seed() + run);
  }
  
  // Perform a parallel for-loop for the different runs
  // The signal handlers needed to be shared, because class members are allways shared
#pragma omp parallel for shared(run_simulations) shared(measurement_accumulator) shared(beta) shared(parameters)
  for (unsigned int run = 0; run < parameters.run_number; ++run)
  {
    // Perform the relaxation steps
    run_simulations[run]->do_metropolis_steps(parameters.relaxation_steps, beta);

    // For each measurement, perform the steps, invoke the signal handler, take the measurement and check for posix signals
    for (unsigned int m = 0; m < parameters.measurement_number && !this->check_for_posix_signal(); ++m)
    {
      run_simulations[run]->do_metropolis_steps(parameters.steps_between_measurement, beta);
...
void MetropolisParallel<ConfigurationType,Step,RandomNumberGenerator>::do_parallel_metropolis_simulation(const Parameters& parameters, InverseTemperatureIterator beta_begin, InverseTemperatureIterator beta_end, AccumulatorIterator measurement_accumulator_begin, AccumulatorIterator measurement_accumulator_end)
{  
  ...
  {
    do_parallel_metropolis_simulation(parameters, *beta_iterator, *measurement_accumulator_iterator);
    if (this->is_terminating) break;
  }
}
