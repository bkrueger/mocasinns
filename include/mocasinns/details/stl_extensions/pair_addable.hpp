...
namespace Mocasinns
{
  namespace Details
  {
	...
	//! Copy constructor
	PairAddable(const PairAddable& pair_addable) : data(pair_addable.first, pair_addable.second), first(data.first), second(data.second) { }
	...
	//! The pair constructor. Constructs a pair such that first is constructed from x and second is constructed from y. 
	PairAddablePlain(const T1& first, const T2& second) :  Base(first, second) { }
	//! Creates an extended pair from the standard pair
	PairAddablePlain(const typename Base::data_type& std_pair) : Base(std_pair) { }
      };
    }
  }
}
