...
namespace Gespinst
{
  ...
  public:
     ...
     */
    RealSpin(value_type_spin_real value) { set_value(value); }
     ...
     */
    bool operator==(const RealSpin& other) const { return (_value == other._value); }
     ...
     * \param other Spin to compare with
     */
    bool operator!=(const RealSpin& other) const { return !operator==(other); }
    //! Operator for assigning a value
    /*!
     * \param value RealSpin that should be assigned to this spin
     */
    void operator=(const value_type_spin_real value) { _value = value; }
    //! Friend operator for multiplying to spins to a double
    friend double operator*(const RealSpin& spin_1, const RealSpin& spin_2);

    //! Create a vector with all different values of ising spins
    /*!
     * \details Returns a STL vector of RealSpin that represents all possible values this spin can have. This is a vector two spins, the first spin up and the second spin down with the value of this spin.
     */
    std::vector<RealSpin> all_possible_values() const
    {
      std::vector<RealSpin> result;
      result.push_back(RealSpin(_value));
      result.push_back(RealSpin(-_value));
      return result;
    }
     ...
     */
    RealSpin random_differ(double) const
    {
      return RealSpin(-_value);
    }
   ...
   */
  inline double operator*(const RealSpin& spin_1, const RealSpin& spin_2)
  {
    return spin_1._value * spin_2._value;
  }

  //! Output stream operator for Real Spin
  inline std::ostream& operator<<(std::ostream& lhs, RealSpin const& rhs)
  {
    lhs << rhs.get_value();
    return lhs;
  }
