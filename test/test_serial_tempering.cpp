...
class TestSerialTempering::ObserveIsingEnergy
{
public:
...
class TestSerialTempering::ObserveIsingEnergyMagnetization
{
public:
...
CppUnit::Test* TestSerialTempering::suite()
{
  CppUnit::TestSuite *suite_of_tests = new CppUnit::TestSuite("TestSerialTempering");
  suite_of_tests->addTest( new CppUnit::TestCaller<TestSerialTempering>("TestSerialTempering: test_do_serial_tempering_steps", &TestSerialTempering::test_do_serial_tempering_steps) );
  suite_of_tests->addTest( new CppUnit::TestCaller<TestSerialTempering>("TestSerialTempering: test_do_serial_tempering_simulation", &TestSerialTempering::test_do_serial_tempering_simulation) );
    
  return suite_of_tests;
}

void TestSerialTempering::setUp()
{
...
void TestSerialTempering::tearDown()
{
  for (unsigned int i = 0; i < 5; ++i)
...
void TestSerialTempering::test_do_serial_tempering_steps()
{
  ...
  {
    n++;
    test_simulation->do_serial_tempering_steps(100, inverse_temperatures.begin(), inverse_temperatures.end());
...
void TestSerialTempering::test_do_serial_tempering_simulation()
{
  // Perform the simulation without accumulator
  std::vector<std::vector<double> > result_vector = test_simulation->do_serial_tempering_simulation<ObserveIsingEnergy>(inverse_temperatures.begin(), inverse_temperatures.end());
  ...
  std::vector<AccumulatorType> acc_default_vector = test_simulation->do_serial_tempering_simulation(acc_default, inverse_temperatures.begin(), inverse_temperatures.end());
  std::cout << std::endl;
  ...
  std::vector<AccumulatorType> acc_vector = test_simulation->do_serial_tempering_simulation<ObserveIsingEnergy>(acc, inverse_temperatures.begin(), inverse_temperatures.end());
  // Test the result
